<script setup>
definePageMeta({ title: 'Mi perfil', requiresAuth: true })

const { user, stats, statsLoading, loadStats, logout } = useAuth()
const initial = computed(() => user?.value?.alias?.charAt(0)?.toUpperCase() || '?')
const agentCode = computed(() => user?.value?.id ? `GB-${String(user.value.id).padStart(4, '0')}` : 'GB-????')
...
function stars(n) {
  const count = typeof n === 'number' && Number.isFinite(n) ? Math.round(Math.max(0, Math.min(5, n))) : 0
  if (count < 1) return ''
